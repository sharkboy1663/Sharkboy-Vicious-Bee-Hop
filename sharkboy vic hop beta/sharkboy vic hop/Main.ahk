...
if WinExist("Sharkboy Vic Hop") {
    MsgBox "Alt script is already running, please exit it before running this script!"
    ExitApp()
}
...
saveSettings(*) {
    if isNumber(MovespeedEdit.Text) {
        IniWrite(MovespeedEdit.Text, settingsPath, "Settings", "Movespeed")
    }
    else {
        MsgBox "Not a valid movespeed!"
        return
    }
    if isValidWebhook(discordWebhookEdit.Text) {
        IniWrite(discordWebhookEdit.Text, settingsPath, "Settings", "Webhook")
    }
    else {
        MsgBox "Not a valid webhook!"
        return
    }
    if (RegExMatch(userIDEdit.Text, "\d{17,19}")){
        IniWrite(userIDEdit.Text, settingsPath, "Settings", "User ID")
    }
    else {
        MsgBox "Not a valid user ID!"
        return
    }
    MsgBox "Succesfully saved all settings!"
}
